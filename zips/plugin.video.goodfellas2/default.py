...
if mode==None:
    ...
    if not url.startswith("plugin://plugin") or not any(x in url for x in g_ignoreSetResolved):#not url.startswith("plugin://plugin.video.f4mTester") :
        ...
        if "first" in url:
            ...
            dp.create("[COLOR=blue][B]Cerebro Prime Scraper[/COLOR][/B]","Connecting to Stream Server","Time can take 5-60 seconds..","Some Channels make stick @ 2% this is ok just wait...")
            dp.update(1)
            xbmc.sleep(1000)
            dp.update(2)

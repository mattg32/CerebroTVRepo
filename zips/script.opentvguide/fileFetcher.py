...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.opentvguide'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_REMOTE:
                xbmc.log('[script.opentvguide] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[script.opentvguide] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                f = open(tmpFile, 'wb')
                xbmc.log('[script.opentvguide] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
            ...
            if os.path.getsize(tmpFile) > 256:
                ...
                xbmc.log('[script.opentvguide] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
            else:
